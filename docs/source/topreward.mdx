...
The processor appends the answer token, label-masks everything except that final token, and the model reads back the log-probability of the literal `"True"`. The resulting `log P("True" | video + prompt + instruction)` is the reward.

Because the method only depends on a frozen VLM, TOPReward is **zero-shot**: there are no fine-tuned weights to host. The "model" in LeRobot is a small wrapper around `transformers`' `Qwen3VLForConditionalGeneration`. The processor owns the tokeniser, builds the full chat prompt, and emits the label mask (EO-1/Robometer pattern).
...
The preprocessor tokenises the full prompt (video + prefix + instruction suffix), appends the answer token, and writes Qwen-VL tensors + `labels` under `observation.topreward.*`. The model reads those tensors and extracts the log-prob reward directly from the precomputed labels.
...
prompt_suffix_template = "{instruction} Decide whether the above statement is True or not. The answer is:"
```

Both are exposed on `TOPRewardConfig` for ablation. The suffix template **must** contain `{instruction}`; the processor appends the final `True` answer token.
