...
EO-1 is a **Vision-Language-Action policy for robot control**. The LeRobot implementation integrates EO-1 with the standard LeRobot training, evaluation, processor, and checkpoint APIs.
...
- State and action padding for heterogeneous robot dimensions
...
This page is intentionally a first complete scaffold for the EO-1 LeRobot docs. The following sections should be filled in as the integration stabilizes:

- Public LeRobot-format checkpoint links
- Final LIBERO and real-robot benchmark tables
- Reproduction commands for released checkpoints
