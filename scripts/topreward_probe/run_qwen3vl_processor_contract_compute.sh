...
echo "proxy_env_present HTTPS_PROXY=$([[ -n "${HTTPS_PROXY:-}" ]] && echo SET || echo unset) HTTP_PROXY=$([[ -n "${HTTP_PROXY:-}" ]] && echo SET || echo unset) https_proxy=$([[ -n "${https_proxy:-}" ]] && echo SET || echo unset) http_proxy=$([[ -n "${http_proxy:-}" ]] && echo SET || echo unset)"

/mnt/hwfile/songhaoming/bin/lerobot-apptainer \
  python scripts/topreward_probe/probe_qwen3vl_processor_contract.py \
