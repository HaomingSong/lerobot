...
The checkpoint can also come straight from a run that pushed with `--save_checkpoint_to_hub`, which is how a training job on remote hardware leaves its checkpoints behind:

```bash
lerobot-convert-dcp --repo_id=${HF_USER}/my_run
lerobot-convert-dcp --repo_id=${HF_USER}/my_run --checkpoint_step=5000 --push_to_hub=${HF_USER}/my_run
```

`--repo_id` downloads the checkpoint into `--output_dir` (by default `outputs/convert_dcp/<repo id>`) and then behaves exactly as if that directory had been passed as `--checkpoint_dir`. It converts the highest step present unless `--checkpoint_step` picks another one, and it fetches `pretrained_model/` only — the optimizer state under `training_state/` is larger than the model itself and conversion never reads it. Note that `--delete_dcp` then removes the downloaded shards only; the ones in the repo stay put.

Only checkpoint pushes carry shards to the Hub. A **published** model repo — whatever `--policy.push_to_hub=true` or `--push_to_hub` above writes — holds safetensors only, so there is nothing in it to convert.
